...
13. **生态不兼容 EOA**（jason 2026-07-10）：cos72 是 **AirAccount-only**（passkey + email，砍掉插件/助记词/手动地址）——EOA 是半成品、UX 差，AirAccount 是一次进化。→ **Phase 0 会话/写路径无 `window.ethereum` fallback**，模块的 window.ethereum 路径是**替换**不是并存。日后若要兼容 EOA，从 **AirAccount 侧加一个 EOA 兼容入口**。
   - **分层定稿（jason 2026-07-10 答开放问题）**：**用户层 = AirAccount-only**（模块 / 转账 / 任务 / 商店 / 投票）；**infra / operator 层 = EOA**（operator 部署、DVT-register、infra 初始化）——因为 infra 参与者（operator / DVT 节点方）**本身没有 AirAccount**，用 EOA 部署密钥初始化。→ YAAA 现有 operator EOA 轨道（`WalletContext` Track C / `lib/sdk/operator.ts`）**保留**、服务 operator/infra；cos72 的 `useCos72Session`（AirAccount）服务用户模块。**两轨并存、按层分**。
14. **JuryContract 复用 DVT 机制**（jason 2026-07-10）：质押/惩罚不自建，**复用 DVT（DVTValidator/BLSAggregator/GTokenStaking.slash）**；若需加 DVT proposal 或配合 jury 重构，**Phase 1 时提需求给 repo:sp（CC）**，不自己在 MyTask 里造。
15. **模块定位纠正**（jason 2026-07-10）：**不是「集成而非新建」**——三模块虽已开发到一定阶段，但**远未完成/产品化**（合约、流程、交互都有大优化空间）。cos72 做的是 **集成 + 深度优化**：可进各源仓目录**改/调合约**、**重建界面**、优化流程/交互。
16. **配置策略 + self-host**（jason 2026-07-10）：**KMS / DVT = 用 SDK 默认（生态锁定，用户不配，属我们独有生态）**；**bundler / RPC = 用户可自配，也可用母站（用母站需 KMS api-key）**。self-host 核心：填好自己的 RPC → 下载 cos72 即可自运行、**零外部依赖**。母站提供一份基础运营**兜底**（有成本 → 收 aPNTs 社区积分；**可用给社区做贡献换积分、不一定花钱**；同逻辑适用其他社区）。aPNTs 支付/订阅抵扣**后续再说**。
17. **auth 并入 KMS**（jason 已述，代码已核）：**无独立 `auth.aastar.io`**，passkey RP / WebAuthn ceremony 由 **`kms.aastar.io`** 承接（AirAccount `kms/host/src/api_server.rs`：`/BeginRegistration`·`/CompleteRegistration`·`/BeginAuthentication`；RP id 默认 `aastar.io`，支持 `*.aastar.io` 通配；`/kms/create-agent-key` 走完 WebAuthn ceremony 后签发 **agent-key Bearer JWT** = 会话 token）。浏览器直做 ceremony（无需 api-key）；后端→KMS 用 **`x-api-key`**（server-to-server，如 DVT 节点）或 **agent-key Bearer JWT**（签名免每次 ceremony）。→ **MyShop permit 迁 KMS = 用 agent-key + `sign_typed_data`**（KMS host 已支持，仅缺 SDK wrapper）。DVT 的 relay / x402 facilitator 挂在 DVT 节点下；KMS 只签名（`SignX402Payment` 只是签）。
18. **移动端 = Flutter**（未来看情况或 RN）；**FangPay = 下游极简支付皮肤**：砍投票/兑换/shop，只承载 AirAccount 登录/注册 + 收款地址表单。现状 = **仅设计文档**（`~/Dev/jhfnetboy/FangPay` + `~/Dev/Brood/research/fangpay`，无实现代码）。收款地址可填普通地址或 AirAccount 地址——**AirAccount 地址享便利+安全，普通地址不享**（对应 `FANGPAY-AASTAR.md` Rail A「EOA+SBT」vs Rail B「AirAccount」）。与「生态不兼容 EOA」不冲突：收款地址是资金**去向**、可为任意地址；但**用 cos72/FangPay 本身需 AirAccount**。⚠️ 此表述是 jason 口述的产品决策，FangPay 仓无逐字文档，先记为决策。
