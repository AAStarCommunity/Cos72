...
13. **生态不兼容 EOA**（jason 2026-07-10）：cos72 是 **AirAccount-only**（passkey + email，砍掉插件/助记词/手动地址）——EOA 是半成品、UX 差，AirAccount 是一次进化。→ **Phase 0 会话/写路径无 `window.ethereum` fallback**，模块的 window.ethereum 路径是**替换**不是并存。日后若要兼容 EOA，从 **AirAccount 侧加一个 EOA 兼容入口**。⚠️ **开放问题（待 jason）**：YAAA 现有 operator/DVT-register 流程用 operator EOA（`WalletContext` Track C）——这些也一律 AirAccount-only，还是保留一个「高级 operator EOA」轨道？
