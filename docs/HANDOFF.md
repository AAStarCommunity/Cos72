...
## 7. YAAA 侧新待办（jason 2026-07-11 提，跨仓）

> 注意：这是 **YAAA 基座仓**（`~/Dev/aastar/YetAnotherAA`）的任务，不是 cos72——但 cos72 fork 会继承。属 **infra/operator 层（EOA 轨道，见 §2 决策 2）**。

1. **无需登录的 infra 初始化页**：YAAA 提供一个**独立目录、不需要登录**的页面，供 infra 初始化用（operator/infra 参与者用自己的 EOA，没有 AirAccount 账户所以不登录）。与「用户层 AirAccount-only / infra 层 EOA」分层一致。
2. **CC-40（repo:yaaa + repo:sdk）= 集成 SDK 的 DVT node-onboarding-portal**：SDK 建了 `node-onboarding-portal`（React/Vite，运行时 `buildDvtPop` 产合法 nodeId 已 E2E 验证；链上 stake+register 由 CC-36 CLI E2E tx `0xe4e1de53…` 证明）。**YAAA 分工**：复制 `node-onboarding-portal/src/`（纯 React，Vite 壳不搬）进 Next；唯一改 `src/lib/sdk.ts` 的钱包接缝换成 YAAA 自己的 provider，其余全走 `@aastar/sdk`；窄子路径导入；依赖 `@aastar/sdk@^0.42.0`。**约束**：operator 自筹（owner key 不入浏览器）、生产密钥走 HSM、KMS-TEE 路径 gated 在 CC-37。
3. 这条与 CC-17（DVT 节点注册向导）同源——YAAA 已有 `app/operator/dvt-register/`（PR#424），CC-40 是升级/对齐 SDK 新 portal + 加无登录 infra-init 入口。

**接手提示**：这块是 YAAA 仓的活，建议在 `~/Dev/aastar/YetAnotherAA` 开会话做（或 cos72 继承后统一）；先确认 SDK `@aastar/sdk@0.42.0` 是否已发 + `node-onboarding-portal` 源码位置。
