...
| MT-4 | 生态 SBT 替换:重部署时 `MYSBT_ADDRESS` 指向 canonical `0x4867…5Aa7`(脚本入口已留;JuryContract 侧 try/catch 天然兼容)                                                                                                                                                          | P1     | ✅ merged MyTask#11                                                                                            |
| MT-5 | 奖励 token 换真 xPNTs + 该 token `addAutoApprovedSpender(taskEscrow)`(需 communityOwner/FACTORY 权限)→ createTask 1 op;删 `createTaskWithPermit`/`acceptTaskWithSignature`(ecrecover,AA 永远不可用);修 `assignSupplier` 误用 `ZeroAmount()` error;`Disputed` 枚举补入口或删除 | P1     | ✅ merged MyTask#10(xPNTs 替换部分移 MT-10 后续)                                                               |
...
| MT-8 | 重部署 Sepolia(MT-1..5 完成后)+ 新地址交付 cos72                                                                                                                                                                                                                              | P1     | ✅ 2026-07-14 escrow `0x171234DD…` jury `0x63f38d99…` SBT=canonical,4 合约 verified,链上核验过(记录 MyTask#12) |
