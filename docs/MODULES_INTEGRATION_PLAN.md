...
| #    | 任务                                                                                                                                                                                                                                                                          | 优先级 | 说明               |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------ |
| MT-1 | **Jury 分成资金黑洞修复**:escrow 把 10% jury share 转给 JuryContract 后永久锁死(合约无任何转出函数)。改 pull 模式:按 task 给投票 juror 记账 `pendingRewards[juror][token]`,juror `claimRewards` 自取;dust 归 owner sweep                                                      | **P0** | 🚧 开工中          |
| MT-2 | **challengeWork 兼容 AA**:现要求原生 ETH `msg.value≥0.01` 质押(gasless 用户无 ETH),退款用 `transfer()`(2300 gas,AA 合约账户必 revert → 任务永久卡死)。改 ERC-20(xPNTs)质押 `transferFrom` + 退款走 ERC-20 transfer                                                            | **P0** | 🚧 开工中          |
| MT-3 | `DeploySepolia.s.sol` + broadcast 记录提交 git(当前部署不可复现)                                                                                                                                                                                                              | **P0** | 🚧 开工中          |
...
| #    | 任务                                                                                                                                                                                                                  | 优先级        | 说明         |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------ |
| MV-0 | **spike:EIP-1271 × Snapshot Hub 验签可行性**(AirAccount `isValidSignature` + 已部署账户实测;未部署需确认 Hub 是否支持 ERC-6492)。**go/no-go:不通过则整条 SSO 投票路线改道**                                           | **P0·最先做** | ⬜           |
| MV-1 | [KMS+cos72] 远程 `signTypedData` API(**限定 snapshot hub domain 白名单**)+ SSO token 签发/校验端点 + CORS 白名单 MyVote 域名                                                                                          | **P0**        | ⬜ 依赖 MV-0 |
...
| MV-5 | [Snapshot 配置,非代码] 独立 space 配 Sepolia(network 11155111)`erc20-balance-of` strategy 指向 xPNTs/GToken;tenants.json/KV spaceId 切换                                                                              | P1            | ⬜           |
