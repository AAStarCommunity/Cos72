...
| **OPR-01** 完整准入（registerRole→deploy xPNTs→paymaster→deposit） | ✅正向 | 🟡 **已建，推进到 register，fixme** | `operator-onboarding.spec.ts`。**进展**：换新 Infura RPC + `withRetry`(receipt 等待) + 注入钱包 sendTx 显式 gas(25/3) 后，**注资 → connect → AOA → 资源预检全部通过**，推进到首个写步骤 **Register Community**。**blocker = registry 合约 revert（非测试代码）**：Step3 `registryActions.registerRole(ROLE_COMMUNITY)` 链上 revert。已用 `simulateContract` **定论**：充足的 fresh EOA（70 GT + 对 GTokenStaking 和 Registry 各 30 allowance）仍**裸 revert "execution reverted"**（无 reason、无具名 error）→ 排除 allowance/spender/余额；而已注册 EOA 同调用能干净解码为 `RoleAlreadyGranted`（证明 ABI/编码对）。**根因 = roleData**：ROLE_COMMUNITY 注册要求 roleData 是编码的社区档案，Step3 传 `"0x"` 才裸 revert。**SDK 已修复发布 0.26.9**（新增 `encodeCommunityRoleData`，[aastar-sdk#169](https://github.com/AAStarCommunity/aastar-sdk/issues/169)）。**YAA 已修**：bump 0.26.9 + Step3 改 `data: encodeCommunityRoleData({name, ensName, stakeAmount})`。✅ **PASS（~1.2m）**——测试稳过 **Register Community（#169 修复）+ Deploy xPNTs**，断言到达 Paymaster 步（证明两笔链上写都成功上链）。测试 rescope 为「register + deploy xPNTs」。Paymaster V4 部署 + EntryPoint deposit 为后续专项 |
